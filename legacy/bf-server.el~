...
(setq bf:server:usage:enabled-p t)

;;; (bf:server:full/update)
(defun bf:server:full/update ()
  ""
  (interactive)
...
(defun bf:server:install/update ()
...
(lambda () "
** [[elisp:(show-all)][(>]] [[elisp:(blee:menu-sel:outline:popupMenu)][+-]] [[elisp:(blee:menu-sel:navigation:popupMenu)][==]]     [[elisp:(lsip-local-run-command "ps ho ppid $$")][ps ho ppid $$]] ~||~  =parent pid is running emacs pid -- it should match:= [[elisp:(emacs-pid)][(emacs-pid)]]  [[elisp:(org-shifttab)][<)]] E|
")
